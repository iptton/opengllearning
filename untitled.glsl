...
void main(void) {
	v_position =u_projectionMatrix * u_modelViewMatrix * a_position + 0.51 * vec4(cos(u_time), 0.0, cos(u_time), -0.5*cos(u_time));
	v_normal = u_normalMatrix * a_normal;
	v_texcoord = a_texcoord;
	v_color = a_color*vec4(0.5333, 0.4392, 0.4392, 0.5);
...
void main() {
	...
	vec3 ambient = vec3(0.9686, 0.9765, 0.9569);
	vec3 direction = vec3(0.1373, 0.1608, 0.1608);
	vec3 lightColor = vec3(0.0745, 0.0078, 0.0078);
